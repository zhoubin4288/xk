<?php
    ...
    //远程抓取图片配置
    $config = array(
        "savePath" => "../../Uploads/ueditor/yuancheng/" ,            //保存路径
	...
	require_once('Db.php');
    getRemoteImage( $uri,$config,$dbconfig );
     ...
     * @param $config
     */
    function getRemoteImage( $uri,$config,$dbconfig)
    {
        //忽略抓取时间限制
        set_time_limit( 0 );
        ...
        foreach ( $imgUrls as $imgUrl ) {
			...
			$arr['originalName'] = rand( 1 , 10000 ) . time();
			$arr['url'] = str_replace('../../','',$tmpName);
			$arr['size'] = $uriSize;
			$arr['type'] = str_replace('.','',$fileType);
